"""
This script will generate a 2d triangular based lattice of points connected by ellipsoids
...
def start(Lx,Ly,filename):
    f=open(filename,'w')
    f.write("""
//2D Triangular Lattice Simulation with ellipsoids
//Version 1.0
//Nathan Shettell
...
\n""")
    ...
    while j<Ly:
        if j%2==0:
            ...
            ellipsoids_x+=[(2*i+2,sqrt(3)*j,0,randint(0,1)) for i in range(Lx-1)]
        j+=1
    ...
    """
    Every list os ellipsoids contains tuples of the form (i,j,theta,colour)
    where (i,j) is the location of the center of the ellipse, theta represents the
    rotation of the ellipse along the x axis and colour is the colour given to the ellipse
